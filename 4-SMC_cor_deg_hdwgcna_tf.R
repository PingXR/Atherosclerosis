...
p
ggsave(
  file.path(outdir, "WTAP-GOBP_SMOOTH_MUSCLE_CELL_MIGRATION1.pdf"),
  plot = p,
  height = 5,
  width = 5
)
...
library(ggpmisc)
p2 <- expr %>%
  ggplot(aes(x = WTAP, y = GOBP_SMOOTH_MUSCLE_CELL_MIGRATION)) +
  ggrastr::rasterise(ggpointdensity::geom_pointdensity(size = 0.5),dpi = 600) +
  geom_smooth(method = "lm",
              formula = y ~ x,
              color = "#2e86c1",
  ...
  labs(y = "GOBP_SMOOTH_MUSCLE_CELL_MIGRATION" |>
         ...
         str_replace_all("_", " ") |>
         str_to_sentence(),
       title = "R = 0.34, P = 1.6e-07",
  )
p2
ggsave(
  file.path(outdir, "WTAP-GOBP_SMOOTH_MUSCLE_CELL_MIGRATION2.pdf"),
  plot = p2,
  height = 4,
  width = 4
)
...
p <- p$data %>%
  ...
  ggplot(aes(x = ident, y =`score_GOBP-SMOOTH-MUSCLE-CELL-MIGRATION`, fill = ident)) +
  geom_boxplot() +
  ggsignif::geom_signif(textsize = 2, comparisons = list(c("low", "high"))) +
  theme_cat() +
  labs(title = "WTAP",
       y = "Smooth muscle cell migration") + theme(aspect.ratio = 2,
                                                   axis.title.x = element_blank()) +
  scale_fill_manual(values = c(low = "#00a6e1",
                               high = "#ee6470")) +
  NoLegend()
p
ggsave("Smooth muscle cell migration.pdf",height =4,width = 4)
adata <-
  readRDS(
    "~/Atherosis_0723/20230204_Atherosis/result/SMC/SMC/k_20/correlation/GO:BP.scores.rds"
  )
...
p <- VlnPlot(
  sub_seurat_obj,
  features = "GOBP-VASCULAR-ASSOCIATED-SMOOTH-MUSCLE-CELL-PROLIFERATION",
  group.by = "WTAP_group",
  assay = "RNA"
)
p 
p <- p$data %>%
  mutate(ident = fct_relevel(ident, c("high", "low"))) %>%
  ggplot(aes(x = ident, y =`score_GOBP-VASCULAR-ASSOCIATED-SMOOTH-MUSCLE-CELL-PROLIFERATION`, fill = ident)) +
  geom_boxplot() +
  ggsignif::geom_signif(textsize = 2, comparisons = list(c("low", "high"))) +
  theme_cat() +
  labs(title = "WTAP",
       y = "Vasular associated smooth muscle cell proliferation") + theme(aspect.ratio = 2,
                                                                          axis.title.x = element_blank()) +
  scale_fill_manual(values = c(low = "#00a6e1",
                               high = "#ee6470")) +
  NoLegend()
p
ggsave("Vasular associated smooth muscle cell proliferation.pdf",height =4,width = 4)
adata <-
  readRDS(
    "~/Atherosis_0723/20230204_Atherosis/result/SMC/SMC/k_20/correlation/GO:BP.scores.rds"
  )
...
p
ggsave(
  file.path(outdir, "WTAP_high-low_ADAMTS1.pdf"),
  plot = p,
  height = 3,
  width = 3
)
...
p
ggsave(
  file.path(outdir, "WTAP_high-low_GNA13.pdf"),
  plot = p,
  height = 3,
  width = 3
)
...
p <- p$data %>%
  mutate(ident = fct_relevel(ident, c("high", "low"))) %>%
  ggplot(aes(x = ident, y = PRKG1, fill = ident)) +
  geom_violin(cex=0.5,color = "black")+
  geom_boxplot(width=0.1,cex=0.5,color = "black",fill = "white") +
  ggsignif::geom_signif(textsize = 2, comparisons = list(c("low", "high"))) +
  theme_cat() +
  labs(title = "WTAP",
       y = "PRKG1") + theme(aspect.ratio = 2,
                            axis.title.x = element_blank()) +
  scale_fill_manual(values = c(low = "#2e86c1",
                               high = "#e32d32")) +
  NoLegend()
p
ggsave(
  file.path(outdir, "WTAP_high-low_PRKG1.pdf"),
  plot = p,
  height = 3,
  width = 3
)
...
pdf("2-all_hubgene.pdf",height = 5, width =6 )
...
print(p)
dev.off()
pdf("3-all_hubgene.pdf",height = 5, width =6 )
ModuleUMAPPlot(
  seurat_obj,
  edge.alpha=0.25,
  sample_edges=TRUE,
